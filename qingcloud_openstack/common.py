...
qcapi_opts = [
    cfg.IntOpt('qcapi_max_limit',
               default=1000,
               help='The maximum number of items returned in a single '
                    'response from a collection resource'),
    cfg.StrOpt('qcapi_compute_link_prefix',
               help='Base URL that will be presented to users in links '
                    'to the OpenStack Compute API'),
    cfg.StrOpt('qcapi_glance_link_prefix',
               help='Base URL that will be presented to users in links '
                    'to glance resources'),
]
CONF = cfg.CONF
CONF.register_opts(qcapi_opts)
...
def limited(items, request, max_limit=CONF.qcapi_max_limit):
...
def get_limit_and_marker(request, max_limit=CONF.qcapi_max_limit):
...
class ViewBuilder(object):
    ...
    def _get_collection_links(self,
        ...
        links = []
        max_items = min(
            int(request.params.get("limit", CONF.qcapi_max_limit)),
            CONF.qcapi_max_limit)
    ...
    def _update_glance_link_prefix(self, orig_url):
        return self._update_link_prefix(orig_url,
                                        CONF.qcapi_glance_link_prefix)

    def _update_compute_link_prefix(self, orig_url):
        return self._update_link_prefix(orig_url,
                                        CONF.qcapi_compute_link_prefix)
